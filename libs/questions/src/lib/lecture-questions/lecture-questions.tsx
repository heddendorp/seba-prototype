...
import { QuestionService } from "@seba/api-services";
...
export const LectureQuestionsReducer = (state: any[], action: {type: string, payload: any}) => {
  switch (action.type) {
    case "ADD_QUESTION":
      return [
        ...state,
...
// handle a question added
  ...
  return (
    <div>
      <h3>Questions</h3>
      <AddQuesionTrigger lecureUnitId={props.lecureUnitId} videoReference={props.videoReference} onNewQuestion={handleQuestionAdded} />
      <Grid container spacing={1} direction="column">
          {questions.map((question) => (
            <Grid item key={question._id}>
              <QuestionListEntry question={question} onQuestionUpdate={handleQuestionUpdated} />
            </Grid>
          ))}
        </Grid>
    </div>
