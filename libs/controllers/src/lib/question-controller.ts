...
import { LectureUnit, Question } from '@seba/models';
...
router.get(
  ...
  passport.authenticate('jwt', { session: false }),
  async (req, res) => {
    const lectureUnit = await LectureUnit.findById(req.params.lectureUnitId).populate({path: 'questions', options: {sort: {timestamp: 1}}});
